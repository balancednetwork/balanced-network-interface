...
export const SolanaWalletOptionsModal = ({ id = MODAL_ID.SOLANA_WALLET_OPTIONS_MODAL }: { id?: MODAL_ID }) => {
  const modalOpen = useModalOpen(id);
  const xConnectors = useXConnectors('SOLANA');
      ...
      <Modal isOpen={modalOpen} onDismiss={onDismiss} maxWidth={360}>
        <ModalContentWrapper>
          {xConnectors.length > 0 ? (
            ...
            <>
              <Typography textAlign="center" margin={'0 0 25px'}>
                No Solana-based wallet detected.
              </Typography>
              <Typography textAlign="center">
                ...
                <Typography
                  ...
                  color="primaryBright"
                  onClick={() =>
                    window.open('https://chromewebstore.google.com/detail/phantom/bfnaelmomeimhlpmgjnjophhpkkoljpa')
                  }
                >
                  <UnderlineText>Phantom</UnderlineText>
                </Typography>{' '}
