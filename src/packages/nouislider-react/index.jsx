...
const areEqual = (prevProps, nextProps) => {
  const { start, step, disabled, range, padding } = prevProps;
  return (
    nextProps.step === step &&
    isEqual(nextProps.start, start) &&
    nextProps.disabled === disabled &&
    isEqual(nextProps.range, range) &&
    isEqual(nextProps.padding, padding)
  );
};
...
const Nouislider = props => {
  ...
  const { start, disabled, range, step, padding } = props;

  useEffect(() => {
    if (slider) {
      updateOptions({ range, step, padding });
      slider.set(start);
      setClickableListeners();
    }
    toggleDisable(disabled);
  }, [start, disabled, range, step, padding]);
